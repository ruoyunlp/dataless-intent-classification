...
class E5Model(BaseInferenceModel):
    def __init__(self, model_type, device) -> None:
        super().__init__()
        self.model = AutoModel.from_pretrained(model_type).to(device)
        self.tokenizer = AutoTokenizer.from_pretrained(model_type)
    ...
    def get_sentence_embeds(self, sents: List[str], norm=True, **kwargs) -> torch.Tensor:
        ...
        out = self.model(input_ids=input_ids, attention_mask=attention_mask)
        embeds = out.last_hidden_state.detach()

        # omit [MASK] token embeddings from averaging
        attention_mask = attention_mask & (input_ids != self.tokenizer.mask_token_id)
        embeds = embeds.masked_fill(~attention_mask[..., None].bool(), 0.0)
        embeds = embeds.sum(dim=1) / attention_mask.sum(dim=1)[..., None]
...
class InstructorModel(BaseInferenceModel):
    def __init__(self, model_type, device) -> None:
        super().__init__()
        self.model = INSTRUCTOR(model_type, device=device)
        self.device = device

    def get_sentence_embeds(self, sents: List[str], norm=True, **kwargs) -> torch.Tensor:
        instr = 'Represent the sentence intent'
        text_instruction_pairs = [[instr, s] for s in sents]
        embeds = self.model.encode(text_instruction_pairs)
